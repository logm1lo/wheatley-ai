    ...
    Livekit__SignalRequest__MessageCase message_case)
{
  ...
  case LIVEKIT__SIGNAL_REQUEST__MESSAGE_LEAVE:
    return "LEAVE";
  default:
    ESP_LOGI(LIVEKIT_LOG, "Unknown request message type %d", message_case);
    return "UNKNOWN";
  }
}
    ...
    Livekit__SignalResponse__MessageCase message_case)
{
  ...
  case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_ROOM_UPDATE:
    return "ROOM_UPDATE";
  default:
    ESP_LOGI(LIVEKIT_LOG, "Unknown response message type %d", message_case);
    return "UNKNOWN";
  }
}
...
void lk_websocket_handle_livekit_response(Livekit__SignalResponse *packet)
{
  ...
  case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_TRICKLE:
  {
    ...
    {
      ESP_LOGI(LIVEKIT_LOG, "skipping tcp ice candidate");
      return;
    }
    ...
    if (!parsed)
    {
      ESP_LOGI(LIVEKIT_LOG, "failed to parse ice_candidate_init");
      return;
    }
    ...
    {
      ESP_LOGI(LIVEKIT_LOG,
               "failed to parse ice_candidate_init has no candidate");
      return;
    }

    ESP_LOGI(LIVEKIT_LOG, "Candidate: %d / %s", packet->trickle->target,
             candidate_obj->valuestring);
    if (xSemaphoreTake(g_mutex, portMAX_DELAY) == pdTRUE)
    {
      if (ice_candidate_buffer != NULL)
      {
        ESP_LOGI(LIVEKIT_LOG, "ice_candidate_buffer is not NULL");
      }
      else
      {
        ESP_LOGI(LIVEKIT_LOG, "buffering ICE candidate");
  ...
  case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_OFFER:
    ESP_LOGI(LIVEKIT_LOG, "%s", packet->offer->sdp);
  ...
  case LIVEKIT__SIGNAL_RESPONSE__MESSAGE_UPDATE:
    break;
  default:
    ESP_LOGI(LIVEKIT_LOG, "Unknown message type received.");
  }
}
                                       ...
                                       void *event_data)
{
  ...
  case WEBSOCKET_EVENT_CONNECTED:
    ESP_LOGI(LIVEKIT_LOG, "WEBSOCKET_EVENT_CONNECTED");
    break;
  case WEBSOCKET_EVENT_DISCONNECTED:
    ESP_LOGI(LIVEKIT_LOG, "WEBSOCKET_EVENT_DISCONNECTED");
#ifndef LINUX_BUILD
    esp_restart();
#endif
    break;
  case WEBSOCKET_EVENT_DATA:
  {
    if (data->op_code != 0x2)
    {
      ESP_LOGD(LIVEKIT_LOG, "Message, opcode=%d, len=%d", data->op_code,
               data->data_len);
      return;
    }
    ...
    {
      ESP_LOGE(LIVEKIT_LOG, "Failed to decode SignalResponse message.");
#ifndef LINUX_BUILD
      esp_restart();
#endif
    }
  ...
  case WEBSOCKET_EVENT_ERROR:
    ESP_LOGI(LIVEKIT_LOG, "WEBSOCKET_EVENT_ERROR");
#ifndef LINUX_BUILD
    esp_restart();
#endif
    break;
...
{
  ESP_LOGI(LIVEKIT_LOG, "Send %s", request_message_to_string(r->message_case));
  ...
  if (len == -1)
  {
    ESP_LOGI(LIVEKIT_LOG, "Failed to send message.");
  }
}
...
void lk_websocket(const char *room_url, const char *token)
{
  ...
  if (g_mutex == NULL)
  {
    ESP_LOGE(LIVEKIT_LOG, "Failed to create mutex.");
    return;
  }
  ...
  ESP_LOGI(LIVEKIT_LOG, "WebSocket URI: %s", ws_uri);
