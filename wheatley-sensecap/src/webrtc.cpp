...
{
  ESP_LOGI(LIVEKIT_LOG, "Setting publisher status to %d", status);
...
static void lk_publisher_onconnectionstatechange_task(PeerConnectionState state,
                                                      void *user_data)
{
  ESP_LOGI(LIVEKIT_LOG, "Publisher PeerConnectionState: %s",
...
{
  ESP_LOGI(LIVEKIT_LOG, "Subscriber PeerConnectionState: %s",
           peer_connection_state_to_string(state));
...
#ifndef LINUX_BUILD
    wheatly_send_audio(publisher_peer_connection);
#endif
...
PeerConnection *lk_create_peer_connection(int isPublisher)
{
  PeerConfiguration peer_connection_config = {
      ...
      {
#ifndef LINUX_BUILD
        wheatly_audio_decode(data, size);
#endif
      },
      .onvideotrack = NULL,
  ...
  {
    ESP_LOGE(LIVEKIT_LOG, "Failed to create peer connection");
    return NULL;
  }
